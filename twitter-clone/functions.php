<?php
	...
	$link = mysqli_connect("shareddb-u.hosting.stackcp.net", "twitter-313333680c", "I(:2^]2],XSC", "twitter-313333680c");
	...
	function displayTweets($type){
		...
		if(mysqli_num_rows($result) == 0){
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				echo "<p><button class='toggleFollow btn-success' data-userId='".$row['userid']."'>";
	...
	function displayUsers(){
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			echo "<p><a class='publicUsers' href='?page=publicprofile&userid=".$row['id']."'>".$row['email']."</a></p>";
